#!/usr/bin/env python3
"""
Модуль движка принудительной синхронизации для приложения синхронизации Microsoft To Do и Kaiten
"""
...
class ForceSyncEngine:
    ...
    def force_sync_all(self) -> None:
        """
        Принудительная синхронизация всех задач без учета хэшей
        """
        logger.info("🔄 Запуск принудительной синхронизации To Do ↔ Kaiten")
        ...
        try:
            ...
            # Обновляем все сопоставленные задачи
            for todo_id, meta in list(mapping.items()):
                ...
                # Обновляем Kaiten из To Do (направление To Do -> Kaiten)
                ...
                logger.debug(f"DEBUG: FORCE SYNC To Do -> Kaiten | Задача '{todo['title']}' | Оригинальная дата: {original_due} | Конвертированная дата: {t_due}")
                
                if self.kaiten_client.update_card(kaiten_id, title=todo["title"], description=t_desc, due_date=t_due):
                    # Обновляем хэши в маппинге
                    updated_card = {
                        "title": todo["title"],
                        "description": t_desc,
                        "due_date": t_due or "",
                        "state": 1 if todo["status"] != "completed" else 2
                    }
                    meta["kaiten_hash"] = compute_kaiten_hash(updated_card)
                    meta["todo_hash"] = compute_todo_hash(todo)
                    mapping[todo_id] = meta

                # Обновляем To Do из Kaiten (направление Kaiten -> To Do)
                ...
                logger.debug(f"DEBUG: FORCE SYNC Kaiten -> To Do | Карточка '{card['title']}' | Оригинальная дата: {k_due} | Конвертированная дата: {converted_due}")
                ...
                # Обновляем хэши
                meta["kaiten_hash"] = compute_kaiten_hash(card)
                fake_todo = {
                    "title": card["title"],
                    "body": {"content": k_desc, "contentType": "text"},
                    "status": k_status
                }
                # Добавляем dueDateTime только если converted_due не None
                if converted_due:
                    fake_todo["dueDateTime"] = {"dateTime": converted_due, "timeZone": "UTC"}
                meta["todo_hash"] = compute_todo_hash(fake_todo)
                mapping[todo_id] = meta

            # Обрабатываем новые задачи (как в обычной синхронизации)
            ...
            # Обрабатываем новые карточки Kaiten
